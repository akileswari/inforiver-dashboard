  ...
  reducers: {
    ...
    updateGridItems: (state, action: PayloadAction<GridItem[]>) => {
      if (Array.isArray(action.payload)) {
        ...
        const snapshot: GridItem[] = state.gridItems.map(item => ({ ...item }));
        state.history.push(snapshot);
        state.currentHistoryIndex = state.history.length - 1;
        console.log("Updated grid items:", state.gridItems);
      } else {
        console.error("Invalid payload type in updateGridItems action:", action.payload);
      }
    },
    updateGridItemSize: (state, action: PayloadAction<{ itemId: string; width: number; height: number }>) => {
